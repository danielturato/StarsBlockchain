        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - messageTime <= 300) {
                if (bitcoinMessage.verify(message, address, signature)) {
                    ...
                    reject(new Error("Unable to verify the message."))
                }
            } else {
                reject(new Error("The time elapsed between when the message was sent and the current time must be" +
                                 " less than 5 minutes. Try again."))
            }
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           let block = self.chain.find(block => block.hash === hash)
            ...
            if (!block) {
                reject(new Error("A block with that hash is not in the blockchain."))
            }
